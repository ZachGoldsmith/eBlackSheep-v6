...
$lookup = 'SELECT * FROM bs_accounts WHERE CitizenID = '.$vmaid.'';
$lresult = mysqli_query($mysqli, $lookup);
$lrow = mysqli_fetch_array($lresult, $lookup);
$username = $lrow['CitizenName'];
...
switch($option) {
    case '5': // gold supply option
        $query4 = "INSERT INTO bs_gold (CitizenID, CitizenName, RequestDate, RequestTime, ReqGold, CommuneStatus) VALUES ('$citizenid','$citizenname','$suptoday','$time','$reqgold','$status')";        
        break;
    default: // normal supply options 1-4
        $query4 = "INSERT INTO bs_supplies (CitizenID, CitizenName, RequestDate, RequestTime, ReqFood, ReqWeapons, ReqGold, CommuneStatus) VALUES ('$citizenid','$citizenname','$suptoday','$time','$reqfood','$reqweapons','$reggold','$status')";      
}
$result4 = mysqli_query($mysqli, $query4) or die("Query failed ($query4) - " . mysqli_error($mysqli));


$requested = $username." Requested Supplies for ".$citizenname;
date_default_timezone_set('America/Los_Angeles');
$date2 = date("Y-m-d");
$time2 = date("h:i:sa T");
$sql="INSERT INTO bs_slogs (Date, Time, CitizenName, Action) VALUES ('$date2', '$time2', '$citizenname', '$requested')";
$result=mysqli_query($mysqli, $sql);
header("Location: soldierinfo.php?id=".$citizenid."&srequested=1");
exit;
}

else {
$citizenname = $row5['CitizenName'];
header("Location: soldierinfo.php?id=".$citizenid."&serror=1");	
exit;
}
?>